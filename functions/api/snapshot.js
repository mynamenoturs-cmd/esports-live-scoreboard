// Cloudflare Pages Function: read-through cache for public scoreboard data.
// Heavy/static traffic stays on Cloudflare; only compact tournament JSON is read from Supabase.
...
export async function onRequestGet({request,env}){
  ...
  const reqUrl=new URL(request.url); const slug=(reqUrl.searchParams.get('slug')||'').slice(0,80);
  ...
  const cache=caches.default; const cacheKey=new Request(`${reqUrl.origin}/api/snapshot?slug=${encodeURIComponent(slug)}`,{method:'GET'}); const hit=await cache.match(cacheKey); if(hit)return hit;
  ...
  try{
    const tournaments=await getJson(`${base}/tournaments?slug=eq.${encodeURIComponent(slug)}&select=${encodeURIComponent(tCols)}&limit=1`,env); const tournament=tournaments?.[0];
    ...
    const response=new Response(JSON.stringify({tournament,games,teams,matches,edge_cached_at:new Date().toISOString()}),{headers:jsonHeaders});
