...
async function loadGames(id){const {data,error}=await supabase.from('match_games').select('id,game_number,team_a_points,team_b_points,winner_id').eq('match_id',id).order('game_number');if(error)throw error;return data||[]}
...
async function paint(){
  ...
  games=await loadGames(m.id);const tm=teamMap(root.teams),s=scoreFor(m,games),target=maxWins(m.best_of||1),ready=Math.max(s.a,s.b)>=target;
  ...
  <div class="mlbb-series-scoreboard"><div class="mlbb-series-team a"><div class="mlbb-series-name">${a}</div>${pips(s.a,target,'a')}</div><div class="mlbb-series-vs">VS</div><div class="mlbb-series-team b"><div class="mlbb-series-name">${b}</div>${pips(s.b,target,'b')}</div></div>
  ${ready?`<div class="mlbb-series-ready">✓ ${s.a>s.b?a:b} cukup kemenangan game · tekan Tamat Perlawanan</div>`:`<div class="mlbb-game-entry"><div class="field"><label>Game ${next} · Kill ${a}</label><input id="mlbb-kill-a" type="number" min="0" inputmode="numeric" value="0"></div><div class="field"><label>Game ${next} · Kill ${b}</label><input id="mlbb-kill-b" type="number" min="0" inputmode="numeric" value="0"></div><div class="span-2 mlbb-winner-actions"><button type="button" class="btn primary" data-mlbb-win="a">◆ ${a} MENANG GAME ${next}</button><button type="button" class="btn magenta" data-mlbb-win="b">◆ ${b} MENANG GAME ${next}</button></div></div>`}
  ${games.length?`<div class="mlbb-game-history">${games.map(g=>`<div class="mlbb-game-row"><span class="chip">G${g.game_number}</span><span><span class="winner">${g.winner_id===m.team_a_id?a:b} menang</span><span class="kills"> · Kill ${g.team_a_points||0}–${g.team_b_points||0}</span></span><span class="chip finished">✓</span></div>`).join('')}</div><div class="actions" style="justify-content:center;margin-top:12px"><button id="mlbb-undo" type="button" class="btn danger small">Undo Game Terakhir</button></div>`:''}
  <div class="subtle small" style="text-align:center;margin-top:10px">Lampu ◆ menunjukkan kemenangan game. Nombor kill hanya statistik setiap game.</div>`;
}
...
async function addGame(side){const m=selectMatch();if(!m||gameFor(m)?.scoring_mode!=='series')return;games=await loadGames(m.id);const s=scoreFor(m,games),target=maxWins(m.best_of||1);if(Math.max(s.a,s.b)>=target)return notice('Siri sudah mempunyai pemenang.','error');const n=(games.at(-1)?.game_number||0)+1,winner_id=side==='a'?m.team_a_id:m.team_b_id,kA=Math.max(0,Number($('#mlbb-kill-a')?.value||0)),kB=Math.max(0,Number($('#mlbb-kill-b')?.value||0));const {error}=await supabase.from('match_games').insert({match_id:m.id,game_number:n,team_a_points:kA,team_b_points:kB,winner_id});if(error)return notice(error.message,'error');await syncMatch(m);notice(`Game ${n}: kill ${kA}-${kB}. Pemenang game direkod berasingan.`);await paint()}
