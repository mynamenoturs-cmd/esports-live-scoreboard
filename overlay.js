...
function bestOfLabel(bundle,m){return bundle.activeGame?.scoring_mode==='goals'?'GOALS':`BO${m.best_of||bundle.activeGame?.default_best_of||1}`}
function teamSeries(bundle,m,side){if(bundle.activeGame?.scoring_mode!=='series')return '';const score=side==='a'?m.team_a_score:m.team_b_score;return `<span class="overlay-team-series">${seriesPips(score,seriesTarget(m),side)}<small>GAME WIN</small></span>`}
...
function singleMatch(bundle,m,station=''){
  ...
  const a=tm[m.team_a_id],b=tm[m.team_b_id],series=bundle.activeGame?.scoring_mode==='series';
  ...
  overlay.innerHTML=`
    ...
    ${series?`<div class="overlay-series-label"><strong>BO${m.best_of||1}</strong><small>FIRST TO ${seriesTarget(m)}</small></div>`:`<div class="overlay-score"><span id="sa">${Number(m.team_a_score||0)}</span><span>—</span><span id="sb">${Number(m.team_b_score||0)}</span></div>`}
...
function multiview(bundle){
  const overlay=document.querySelector('#overlay'),tm=teamMap(bundle.teams),matches=multiviewMatches(bundle),series=bundle.activeGame?.scoring_mode==='series';
  ...
  overlay.innerHTML=matches.map(m=>{
    ...
    const aScore=series?seriesPips(m.team_a_score,seriesTarget(m),'a','compact'):`<strong>${Number(m.team_a_score||0)}</strong>`;
    const bScore=series?seriesPips(m.team_b_score,seriesTarget(m),'b','compact'):`<strong>${Number(m.team_b_score||0)}</strong>`;
    return `<section class="overlay-station-card ${isLive?'is-live':'is-scheduled'}">
      ...
      <div class="overlay-station-round">${esc(m.round_name||'Perlawanan')} · ${esc(bundle.activeGame?.code?.toUpperCase()||'GAME')}${series?` · FIRST TO ${seriesTarget(m)}`:''}</div>
    </section>`;
  }).join('');
}

function paint(){const bundle=filterBundle(root,selectedCode),station=selectedStation(),mode=selectedMode();if(mode==='multiview')return multiview(bundle);const matches=station?bundle.matches.filter(x=>stationNo(x)===String(station)):bundle.matches;const m=matches.find(x=>x.status==='live')||matches.find(x=>x.status==='scheduled');singleMatch(bundle,m,station)}
