...
function roundRobin(ids){
  ...
  const rounds=[]; const n=list.length;
  for(let r=0;r<n-1;r++){
    const pairs=[];
...
async function markBye(row,winnerId){
  ...
  const {error}=await supabase.from('matches').update({status:'finished',winner_id:winnerId,finished_at:finishedAt}).eq('id',row.id);
  if(error) throw error;
  if(row.next_match_id&&row.next_match_slot){
    const field=row.next_match_slot==='a'?'team_a_id':'team_b_id';
    const {error:nErr}=await supabase.from('matches').update({[field]:winnerId}).eq('id',row.next_match_id);
    if(nErr) throw nErr;
  }
}
...
async function buildKnockout(bundle,gameId,{pairing='random',bestOf=1,startAt=null,intervalMinutes=20,replaceScope='all',customPairs=null,stationCount:stationCountOption}={}){
  ...
  const offsets=defs.map((_,i)=>defs.slice(0,i).reduce((s,d)=>s+d.count,0));
...
export async function generateGroups(bundle,gameId,{pairing='random',groupCount=4,bestOf=1,startAt=null,intervalMinutes=20,stationCount:stationCountOption}={}){
  ...
  teams.forEach((t,i)=>{
    ...
    const gi=pairing==='seeded'&&cycle%2===1?groupsN-1-pos:pos;
    groups[gi].push(t);
  });
...
export async function generateKnockoutFromGroups(bundle,gameId,{bestOf=1,startAt=null,intervalMinutes=20,stationCount:stationCountOption}={}){
  ...
  const labels=[...new Set(groupMatches.map(m=>m.group_name))].sort();
  if(![2,4].includes(labels.length)) throw new Error('Knockout automatik Top 2 menyokong 2 atau 4 kumpulan.');
  const top={};
  for(const label of labels){
    const ms=groupMatches.filter(m=>m.group_name===label);
    const ids=[...new Set(ms.flatMap(m=>[m.team_a_id,m.team_b_id]).filter(Boolean))];
    const teams=bundle.teams.filter(t=>ids.includes(t.id));
    const standings=calculateStandings(teams,ms);
    if(standings.length<2) throw new Error(`Kumpulan ${label} belum mempunyai Top 2 yang sah.`);
    top[label]=standings.slice(0,2).map(s=>s.team_id);
  }
  let pairs;
...
function installStationSelector(){
  ...
  const interval=document.querySelector('#generator-interval');
  const form=document.querySelector('#generator-form');
  if(!interval||!form)return;
  ...
  const restore=()=>{
    const gid=document.querySelector('#generator-game')?.value||'default';
    try{select.value=localStorage.getItem(`esports-stations-${gid}`)||'1'}catch{select.value='1'}
  };
  restore();
  select.addEventListener('change',()=>{const gid=document.querySelector('#generator-game')?.value||'default';try{localStorage.setItem(`esports-stations-${gid}`,select.value)}catch{}});
  document.querySelector('#generator-game')?.addEventListener('change',restore);
}
if(typeof document!=='undefined'){
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installStationSelector,{once:true});
  else queueMicrotask(installStationSelector);
}
